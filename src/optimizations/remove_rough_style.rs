...
fn remove_rough_style_from_node(node: Node) -> Node {
    match node {
        Node::RegularNode {
            node_type,
            namespace,
...
/// Check if two path elements have similar path data
fn paths_are_similar(d1: &str, d2: &str, tolerance: f64) -> bool {
    let cmd_re = regex!(r"[MLHVCSQTAZ][^MLHVCSQTAZ]*");

    let commands1: Vec<&str> = cmd_re.captures_iter(d1)
        .map(|caps| caps.get(0).unwrap().as_str())
        .collect();
    let commands2: Vec<&str> = cmd_re.captures_iter(d2)
        .map(|caps| caps.get(0).unwrap().as_str())
        .collect();
    ...
    // All corresponding commands must be similar
    for (cmd1, cmd2) in commands1.iter().zip(commands2.iter()) {
        if !commands_are_similar(cmd1, cmd2, tolerance) {
            return false;
        }
    }
...
pub(crate) fn remove_rough_style(nodes: Vec<Node>) -> Vec<Node> {
    let processed_nodes: Vec<Node> = nodes.into_iter().map(remove_rough_style_from_node).collect();

    // Remove consecutive duplicate path elements
    let mut result = Vec::new();
    let mut i = 0;

    while i < processed_nodes.len() {
        let current = &processed_nodes[i];

        // Check if this is a path element
        if let Node::RegularNode { node_type, attributes, .. } = current
            && matches!(node_type, RegularNodeType::Path)
            && let Some(d_attr) = attributes.iter().find(|a| a.name.local_name == "d")
        {
            // Keep current path
            result.push(processed_nodes[i].clone());

            // Skip similar consecutive paths
            let mut j = i + 1;
            while j < processed_nodes.len() {
                if let Node::RegularNode { node_type: next_type, attributes: next_attrs, .. } = &processed_nodes[j]
                    && matches!(next_type, RegularNodeType::Path)
                    && let Some(next_d_attr) = next_attrs.iter().find(|a| a.name.local_name == "d")
                    && paths_are_similar(&d_attr.value, &next_d_attr.value, 0.1)
                {
                    // Skip this similar path
                    j += 1;
                    continue;
                }
                // Not a similar path, stop skipping
                break;
            }
        ...
        // Not a path element or no 'd' attribute, keep it
        result.push(processed_nodes[i].clone());
        i += 1;
    }
